...
export default function MapView({
  ...
  // Removed comparison bar state; handled by MainPage
  ...
  // Default to heat map view
  const [viewMode, setViewMode] = useState<"boundaries" | "heat">(
    defaultViewMode || "heat"
  );
  const isAdmin =
  ...
  useEffect(() => {
    fetchOpportunityGeoJSON()
      .then((gj) => {
        ...
        setFiltered(gj);
        const n = gj.features
          .map((f: any) => getSubzoneName(f.properties))
          .filter(Boolean);
        ...
        const regions = Array.from(regionsSet).sort((a, b) =>
          a.localeCompare(b)
        );
        for (const k of Object.keys(regionIndex))
          regionIndex[k].sort((a, b) => a.localeCompare(b));
        onRegionsLoaded?.(regions);
        onRegionIndexLoaded?.(regionIndex);

        // Build rank buckets
        const buckets: Record<"10" | "20" | "50", string[]> = {
          "10": [],
          "20": [],
          "50": [],
        };
  ...
  function geometryBBox(
    ...
    if (geometry.type === "Polygon") {
      ...
      const polys = geometry.coordinates as number[][][][];
      for (const poly of polys)
        for (const ring of poly) for (const p of ring) update(p[0], p[1]);
    } else {
      return null;
    }
    if (
  ...
  // Removed Add/Remove to Comparison handlers; handled by MainPage
  ...
  function handleSearch(name: string) {
    ...
    const feat = raw.features.find(
      (f: any) => getSubzoneName(f.properties) === key
    );
    if (!feat) return;
    const fc: FeatureCollection<Geometry> = {
      type: "FeatureCollection",
  ...
  useEffect(() => {
    ...
    if (regionName) {
      ...
      feats = feats.filter(
        (f: any) =>
          String(getPlanningAreaName(f?.properties ?? {})).toLowerCase() === rn
      );
    }
    if (rankTop) {
      ...
      feats = feats.filter(
        (f: any) =>
          Number(
            (f?.properties ?? {}).H_rank ?? (f?.properties ?? {}).h_rank
          ) <= top
      );
    }
    const fc: FeatureCollection<Geometry> = {
  ...
  // If defaultViewMode changes, update viewMode (for prop changes)
  React.useEffect(() => {
  ...
  return (
    <div style={{ height: "100%", position: "relative" }}>
      ...
      >
        ...
        {/* Custom 1/2 control stacked above zoom, matching zoom control style */}
        <div
          style={{
            position: "absolute",
            bottom: 112, // 80px (zoom control height) + 8px (gap) + 24px (margin)
            right: 10,
            zIndex: 1000,
            display: "flex",
        ...
        {hawkersInSelected && hawkersInSelected.features.length > 0 && (
          <HawkerCentresLayer
            key={`hawkers-${selectedId ?? "none"}`}
            data={hawkersInSelected}
          />
        )}
        {mrtInSelected && mrtInSelected.features.length > 0 && (
          <MrtExitsLayer
            key={`mrt-${selectedId ?? "none"}`}
            data={mrtInSelected}
          />
        )}
        {busInSelected && busInSelected.features.length > 0 && (
          <BusStopsLayer
            key={`bus-${selectedId ?? "none"}`}
            data={busInSelected}
          />
        )}
      </MapContainer>

      {/* Views dropdown (top-right) removed as requested */}

      {/* Comparison button and tray removed; handled by MainPage */}

      {/* Comparison Bar removed; handled by MainPage */}
    </div>
  );
}
