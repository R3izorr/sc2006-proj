...
export default function ChoroplethLayer({ data, selectedId, onSelect }: Props){
  ...
  const effectiveSelectedId = isControlled ? (selectedId ?? null) : internalSelectedId
  ...
  React.useEffect(() => { selectedRef.current = effectiveSelectedId }, [effectiveSelectedId])
  ...
  const style = (feature?: Feature<Geometry>): PathOptions => {
    ...
    const selected = effectiveSelectedId && id && String(id) === String(effectiveSelectedId)
    return selected
  ...
  const applyStyles = () => {
    layersRef.current.forEach((layer, id) => {
      const isSelected = selectedRef.current && String(id) === String(selectedRef.current)
      const s = isSelected
        ? { ...baseStyle, weight: 3, color: '#fd06b3ff', fillOpacity: 0 }
        : baseStyle
      try { layer.setStyle(s) } catch {}
    })
  }
  ...
  const onEachFeature = (feature: any, layer: any) => {
    ...
    layer.on('mouseover', () => {
      ...
      const isSelected = key && selectedRef.current && key === String(selectedRef.current)
      if (!isSelected) {
    ...
    layer.on('mouseout', () => {
      ...
      const isSelected = key && selectedRef.current && key === String(selectedRef.current)
      if (isSelected) {
        layer.setStyle({ ...baseStyle, weight: 3, color: '#fd06b3ff', fillOpacity: 0 })
      } else {
